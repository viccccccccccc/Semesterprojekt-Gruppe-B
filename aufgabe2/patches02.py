...
class CustomDataset(Dataset):
    ...
    def normalize(self):
        if os.path.exists("scaler_xDECON3.joblib") and os.path.exists("scaler_yDECON3.joblib"):
            self.scaler_x = jl.load("scaler_xDECON3.joblib")
            ...
            for idx in range(len(self.key_list)):
                ...
                self.scaler_y.partial_fit(np.log1p(y.reshape(-1, 1)))

            jl.dump(self.scaler_x, "scaler_xDECON3.joblib")
    ...
    def denormalize_y(self, y):
        if self.normalized:
            y = np.expm1(self.scaler_y.inverse_transform(y.reshape(-1, 1)).flatten())
        return y
    ...
    def __getitem__(self, idx):
        ...
        if self.normalized:
            ...
            y = self.scaler_y.transform(np.log1p(y.reshape(-1, 1))).flatten()
...
class SubNet(nn.Module):
    def __init__(self):
        ...
        nn.LeakyReLU(),
        nn.ConvTranspose2d(32,16, kernel_size = 4, stride = 2, padding = 1), #4
        nn.LeakyReLU(),
        nn.ConvTranspose2d(16,8, kernel_size = 6, stride = 4, padding = 1), #16
        nn.LeakyReLU(),
        nn.ConvTranspose2d(8,1, kernel_size = 6, stride = 4, padding = 1), #64
        nn.LeakyReLU(),
        )
...
class MainNet(nn.Module):
    def __init__(self, num_subnets=16):
