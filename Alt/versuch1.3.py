...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(anz_input, 256),
            nn.LeakyReLU(),
            nn.Linear(256, 128),
            nn.LeakyReLU(),
            nn.Linear(128, 64),
            nn.LeakyReLU(),
            nn.Linear(64, anz_output)
        )
...
def train(train_loader, test_loader):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            now=lastTime
            lastTime = datetime.datetime.now()
            timediff = lastTime - now
            seconds = timediff.total_seconds()
            remainigSeconds = seconds * (epochs-epoch)/5
            print(
                f'Epoch: {epoch + 1}/{epochs}, Train Loss: {avg_train_loss},'
                f' Test Loss: {avg_test_loss}, Learning Rate: {optimizer.param_groups[0]["lr"]}, Time left in sec.: {remainigSeconds}')
            model.train()
